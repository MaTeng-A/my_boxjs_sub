        ...
        if (gpsData && gpsTimestamp) {
            try {
                ...
                if (timeDiff < 3 * 60 * 60 * 1000) {
                    ...
                    console.log("📝 GPS定位数据已过期");
                }
            } catch (e) {
                console.log("❌ GPS定位数据解析失败");
            }
        }
...
function handleDualNotification(weatherData, province, city, district) {
    ...
    // 先发送当天天气通知
    processTodayWeather(weatherData, province, city, district, "今日天气宜人，愿您心情舒畅。");
    
    // 延迟1秒后发送明日天气预报
    setTimeout(() => {
        console.log("⏰ 延迟1秒后发送明日天气预报");
